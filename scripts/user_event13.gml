...
switch(muno_event_type)
{
    ...
    case 6: // css draw
        ...
        break;
    default:
        print("PHONE ERROR: bad event #" + string(muno_event_type));
        break;
}
...
phone = {
    ...
    last_text_size: { width:0, height:0 }, //size of last text drawn using textDraw
    scroll_dist: 0,
...
#define pho_initApp(app_name, app_color, app_color_dark, app_content)
{
    array_push(apps, {
        name: app_name,
        color: make_color_rgb(color_get_blue(app_color), 
                              color_get_green(app_color), 
                              color_get_red(app_color)),
...
// Online fast graphics hint
if (phone_online && get_gameplay_time() < 300)
{
    draw_debug_text(10, 48, "ONLINE: Press the F1 key to enable Fast Graphics.");
}

// Phone activation hint
if (!phone.has_opened_yet && phone_practice)
{
    ...
    draw_sprite_ext(sprite_get("_pho_icon"), 0, x_pos + phone.last_text_size.width - 4, y_pos - 13, 2, 2, 0, c_white, 1);
}
...
if (phone.app == phone.APP_HOME)
{
    for (var i = 1; i < array_length(phone.apps); i++)
    {
        ...
        var app_x = phone_x + 2 + app_sel * phone.cursor_change_timer
                     + ease_backIn(0, phone.screen_width, phone.app_icon_slide_timer, phone.app_icon_slide_timer_max, 1);
...
if (phone.big_screen_pos_offset < 1)
{
    ...
    else if (phone.app == phone.APP_CHEATS || phone.app == phone.APP_UTILS)
    {
        ...
        if (num_options > 1)
        {
            // Split options into multiple columns if needed
            var col_height = 10;
            ...
            for (var j = 0; j < num_cols; j++)
            {
                ...
                for (var i = j * col_height; i < (j + 1) * col_height && i < num_options; i++)
                {
                    ...
                    textDraw(col_x, col_y, "fName", text_color, 18, this_w, fa_center, 1, 0, 1, to_draw, true);
                    col_y += phone.last_text_size.height + 10;
                }
            }
        }
        else
...
{
    //Tags all other characters in this match with three shortcuts
    if (!phone_char_ided)
    {
        with (oPlayer) if (self != other)
        {
            ...
            if (muno_char_id == other.muno_char_id && muno_char_id != noone) //redundant?
            || ("url" in self && url == other.url)
            {
    ...
    // general update utils - cooldown counting down
    if (phone_arrow_cooldown > 0) phone_arrow_cooldown--;
    if (phone_invis_cooldown > 0) phone_invis_cooldown--;

    // all states that can reset a "once per airtime" move cooldown
    ...
    if (state == PS_ATTACK_AIR || state == PS_ATTACK_GROUND)
    {
        ...
        phone_window_end = floor(get_window_value(attack, window, AG_WINDOW_LENGTH)
                             * ((get_window_value(attack, window, AG_WINDOW_HAS_WHIFFLAG) && !has_hit) ? 1.5 : 1));
    }
    // Interrupt attack sounds
    else
    {
        if (phone_attacking)
    ...
    if (phone_attacking)
    {
        ...
        if (phone_using_invul && !phone_invul_override) 
        && (array_find_index(muno_invul_checked, attack) != -1)
        {
            super_armor = false;
            invincible = false;
            soft_armor = 0;
    ...
    // Visible while respawning
    if (phone_practice && state == PS_RESPAWN) visible = true;
    ...
    if (phone.supports_fast_graphics)
    {
        ...
        if !phone_fast && ((!phone_online && fps_real < 60) 
        || (phone_online && keyboard_key == 112)) && !phone.state && !phone.dont_fast 
        && state != PS_SPAWN && (state != PS_IDLE || state_timer > 5)
        {
...
#define initIndexes // Version 0
    ...
    AG_MUNO_ATTACK_REFRESH = i; i++;        // [UNUSED] Set to 1 to refresh this move's data every frame while the frame data guide is open
...
#define pullAttackValue(atk_index, index, def) // Version 0
